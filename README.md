...
| 文档 | 内容 |
|---|---|
| **[提升匹配率方法汇总](docs/提升匹配率方法汇总.md)** | ★ 全部方法、收益、边界与坑 |
| [实验记录](docs/实验记录.md) | 26 节完整实验过程（含所有负面结论） |
...
| 改法 | 是否生效 | 说明 |
|---|---|---|
| **网页配置抽屉里改 → 点「保存配置」** | ✅ **立即生效** | 推荐 |
| **直接编辑 `web_demo_categories.json` → 重启服务** | ✅ 生效 | |
| 直接编辑 json，**不重启** | ⚠️ 不生效 | 但页面会自动弹提示条，点「重新载入」即可 |
| 改 `web_demo.py` 里的 `DEFAULT_CONFIG` | ❌ 不生效 | 那是兜底值，只在 json 不存在时用 |

**外部修改检测**：服务会记录配置文件的 (mtime, size)，页面前端每 4 秒轮询一次
`/api/status`。一旦发现文件被其他程序改过，顶部弹出橙色提示条：

```
⚠ 配置文件已被外部修改
   web_demo_categories.json 在服务启动后被其他程序修改过。当前用的是内存里的旧版本。
                                        [重新载入]  [×]
```

点「重新载入」→ 调 `/api/reload` → 重读配置 + 清空原型缓存，**不用重启服务**。

#### 缓存失效规则

| 你改了什么 | 文本原型重建？ | 图像特征失效？ |
|---|---|---|
| 分类名 / 描述 | ✅ 会（指纹含类别表+模板） | ❌ 不会（与类别无关） |
| **阈值 / margin** | ❌ 不会 | ❌ 不会 |
| 换模型 | ✅ 会（指纹含模型 repo） | ✅ 会（键含模型标识） |
