...
Height: `h-16` (64px), `fixed top-0 left-0 right-0 z-30`
...
- No side margin (full-width, no sidebar offset)
- Padding: `p-4 sm:p-6` (responsive)
- Top reserve: `pt-16` (clears top bar)
...
| Agent cards (mobile-first) | `grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6` |
...
Component: `src/components/StatusBadge.vue`
...
Component: `src/components/StatCard.vue`
...
Component: `src/components/ThemeToggle.vue`
...
Component: `src/components/AIStatusIndicator.vue`
...
Component: `src/components/TaskFilters.vue`
...
| `h-5 w-5` | Card/list item, bottom nav |
...
## 13. Status Colors (CSS classes in `index.css`)
...
All pages and components **must** follow these rules to ensure correct mobile display.

### 14.1 Touch Targets

Interactive elements (buttons, nav items, list rows) must have a minimum clickable area of **44×44px**:
...
Bottom nav items must each be `flex-1` with `min-h-[56px]` to ensure reachable touch areas.

### 14.2 Responsive Padding

Content area padding must be responsive:
...
Never use fixed `p-6` without the mobile-first `p-4` prefix.

### 14.3 iOS Safe Area

Bottom navigation must accommodate the iOS Home Indicator:
...
### 14.4 3D Card on Touch Devices

`.board-card-3d` hover tilt **must** be disabled on touch-only devices:
...
### 14.5 Typography Minimum

Body text must never be smaller than `text-sm` (14px) on mobile. `text-xs` (12px) is only permitted for badges, labels, and helper text.

### 14.6 Layout Rules

- All layouts start single-column (`grid-cols-1`) and expand at `sm:` / `lg:` breakpoints
- Full-width content: never add `ml-*` offset for navigation
...
8. **Mobile-first** — All layouts start from single-column full-width. Touch targets ≥ 44px.
